...
function safeClearTimeout(id) {
    if (id) { clearTimeout(id); activeTimers.delete(id) }
}

function animateBucket(fillFraction) {
    let path = document.getElementById("path")
    let time = 0.0
    animationInterval = setInterval(() => {
        time += 0.1
        let height = (fillFraction !== undefined ? fillFraction : (current/total))*140 + 30
...
async function startSlideShow(root) {
        ...
        for (const slide of slides) {
            if (slide.format == 'video') {
                ...
                if (slide.type === 'long') {
                    ...
                    timeout = safeSetTimeout(() => nextSlide(vidDiv), settings.videoSplittingTime*1000)
                }
                ...
                const timeout = safeSetTimeout(() => nextSlide(imgDiv), jitter(settings.imageInterval*1000))
                imgDiv.onclick = () => {
                    safeClearTimeout(timeout)
                    nextSlide(imgDiv)
                }
                ...
                const timeout = safeSetTimeout(() => nextSlide(iframeDiv), jitter(settings.imageInterval*1000))
                iframeDiv.onclick = () => {
                    safeClearTimeout(timeout)
                    nextSlide(imgDiv)
                }
    ...
    async function nextSlide(elemRemoved) {
        ...
        safeClearTimeout(debounceTimer)
        debounceTimer = safeSetTimeout(loadMoreSlides, 100)
    }
...
function stopSlideShow() {
    ...
    document.getElementById("slideshowBack").style.display = 'none'
    document.getElementById("load-container").style.display = 'none'
...
function showRedgifsForm() {
    for (let elem of document.getElementsByClassName("noForm")) {
        elem.style.display = 'none'
    }
    document.getElementById("redgifsForm").style.display = null
}

function showWelcome() {
    document.getElementById("redditForm").style.display = 'none'
    document.getElementById("e621Form").style.display = 'none'
    document.getElementById("redgifsForm").style.display = 'none'
    for (let elem of document.getElementsByClassName("noForm")) {
...
window.onload = () => {
    ...
    document.getElementById("browseRedgifs").onclick = showRedgifsForm
    document.getElementById("redgifsSubmit").onclick = openRedgifs
    document.getElementById("redgifsBack").onclick = showWelcome
    document.getElementById("slideshowBack").onclick = stopSlideShow
